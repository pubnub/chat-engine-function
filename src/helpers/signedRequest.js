...
export default (request, path, options = {}) => {
  ...
  const params = Object.keys(options)
    .sort()
    .map(k => `${encodeURI(k)}=${encodeURI(options[k])}`)
    .join("&");
  ...
  return vault
    .get("secretKey") // get the PubNub account secret key from PubNub vault
    .then(secretKey =>
      crypto.hmac(
        base64Codec.btoa(secretKey),
        signString,
        crypto.ALGORITHM.HMAC_SHA256
      )
    )
    .then(signature =>
      xhr.fetch(`${pnUrl}${path}?${applySignature(options, signature)}`)
    );
};
