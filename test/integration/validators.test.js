...
describe("validators", () => {
  ...
  describe("#channel", () => {
    ...
    it("should not be empty", done => {
      channel()
        ...
        .catch(err => {
          const expected = {
            status: 401,
            message: "Chat channel must be a string"
          };
          assert.deepEqual(err, expected);
          done();
        });
    ...
    it("should not be too long", done => {
      const ch1 =
        "aswakkakakkskakaaasasaslkkakakaaasaaaaiaakakakazkakakbbbakwkakakakw00e00a0a0a0alslallalammaiwkkakakaka";
    ...
    it("should be private equal to (true or false)", done => {
      ...
      chat(myChat)
        ...
        .catch(err => {
          const expected = {
            status: 401,
            message: "Must supply a chat isPrivate value."
          };
          assert.deepEqual(err, expected);
          done();
        });
    ...
    it("should be group equal to (rooms, system, custom)", done => {
      ...
      chat(myChat)
        ...
        .catch(err => {
          const expected = {
            status: 401,
            message: "Must supply a chat group."
          };
          assert.deepEqual(err, expected);
          done();
        });
    ...
    it("should be included a meta object", done => {
      ...
      chat(myChat)
        ...
        .catch(err => {
          const expected = {
            status: 401,
            message: "Invalid meta object supplied."
          };
          assert.deepEqual(err, expected);
          done();
        });
